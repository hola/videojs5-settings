...
vjs.registerComponent('PopupMenu', vjs.extend(Menu, {
    ...
    constructor: function(player, options){
        ...
        var opt_copylog = vjs.mergeOptions({label: 'Copy debug info', copy: true});
        this.addChild(new LogButton(player, opt_copylog));
...
vjs.registerComponent('LogButton', vjs.extend(MenuItem, {
    constructor: function(player, options){
        MenuItem.call(this, player, options);
        var player_ = player;
        var is_copy = options.copy;
        this.on('click', function(){
            // XXX alexeym: make it work without cdn
            player_.trigger({type: is_copy ? 'copy_logs' : 'save_logs'});
            this.selected(false);
        });
    }
}));
